// 20: [1024,3072] Q8_0 fused GEMV — pure MLIR GPU dialect
hal.executable public @decode_dispatch_20 {
  hal.executable.variant public @cuda_nvptx_fb target(<"cuda", "cuda-nvptx-fb", {iree_codegen.target_info = #iree_gpu.target<arch = "sm_87", features = "+ptx74", wgp = <compute = fp64|fp32|fp16|int64|int32|int16|int8, storage = b64|b32|b16|b8, subgroup = shuffle|arithmetic, dot = dp4xi8toi32, mma = [], subgroup_size_choices = [32], max_workgroup_sizes = [1024, 1024, 1024], max_thread_count_per_workgroup = 1024, max_workgroup_memory_bytes = 166912, max_workgroup_counts = [2147483647, 65535, 65535]>>}>) {
    hal.executable.export public @decode_dispatch_20_matmul_1024x1x3072_f16 ordinal(0)
    ...
    builtin.module {
      func.func @decode_dispatch_20_matmul_1024x1x3072_f16()
        ...
        %c3342336 = arith.constant 3342336 : index
        ...
        %c18432_bytes = arith.constant 18432 : index
        %c0_bytes = arith.constant 0 : index
        ...
        %input = hal.interface.binding.subspan layout(#hal.pipeline.layout<constants = 2, bindings = [#hal.pipeline.binding<storage_buffer, ReadOnly>, #hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) binding(1) alignment(64) offset(%c18432_bytes) flags("ReadOnly|Indirect") : memref<3072xf16, strided<[1], offset: ?>, #gpu.address_space<global>>
        %output = hal.interface.binding.subspan layout(#hal.pipeline.layout<constants = 2, bindings = [#hal.pipeline.binding<storage_buffer, ReadOnly>, #hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) binding(2) alignment(64) offset(%c0_bytes) flags(Indirect) : memref<1024xf16, strided<[1], offset: ?>, #gpu.address_space<global>>

        %tid = gpu.thread_id x upper_bound 256
        %bid = gpu.block_id x upper_bound 128

        %smem = memref.alloc() : memref<3072xf16, #gpu.address_space<workgroup>>
        %_sv0 = memref.load %input[%tid] : memref<3072xf16, strided<[1], offset: ?>, #gpu.address_space<global>>
        memref.store %_sv0, %smem[%tid] : memref<3072xf16, #gpu.address_space<workgroup>>
        %_si1 = arith.addi %tid, %c256 : index
        %_sv1 = memref.load %input[%_si1] : memref<3072xf16, strided<[1], offset: ?>, #gpu.address_space<global>>
        memref.store %_sv1, %smem[%_si1] : memref<3072xf16, #gpu.address_space<workgroup>>
        %_si2 = arith.addi %tid, %c512 : index
        %_sv2 = memref.load %input[%_si2] : memref<3072xf16, strided<[1], offset: ?>, #gpu.address_space<global>>
        memref.store %_sv2, %smem[%_si2] : memref<3072xf16, #gpu.address_space<workgroup>>
        %_si3 = arith.addi %tid, %c768 : index
        %_sv3 = memref.load %input[%_si3] : memref<3072xf16, strided<[1], offset: ?>, #gpu.address_space<global>>
        memref.store %_sv3, %smem[%_si3] : memref<3072xf16, #gpu.address_space<workgroup>>
        %_si4 = arith.addi %tid, %c1024 : index
        %_sv4 = memref.load %input[%_si4] : memref<3072xf16, strided<[1], offset: ?>, #gpu.address_space<global>>
        memref.store %_sv4, %smem[%_si4] : memref<3072xf16, #gpu.address_space<workgroup>>
        %_si5 = arith.addi %tid, %c1280 : index
        %_sv5 = memref.load %input[%_si5] : memref<3072xf16, strided<[1], offset: ?>, #gpu.address_space<global>>
        memref.store %_sv5, %smem[%_si5] : memref<3072xf16, #gpu.address_space<workgroup>>
        %_si6 = arith.addi %tid, %c1536 : index
        %_sv6 = memref.load %input[%_si6] : memref<3072xf16, strided<[1], offset: ?>, #gpu.address_space<global>>
        memref.store %_sv6, %smem[%_si6] : memref<3072xf16, #gpu.address_space<workgroup>>
        %_si7 = arith.addi %tid, %c1792 : index
        %_sv7 = memref.load %input[%_si7] : memref<3072xf16, strided<[1], offset: ?>, #gpu.address_space<global>>
        memref.store %_sv7, %smem[%_si7] : memref<3072xf16, #gpu.address_space<workgroup>>
        %_si8 = arith.addi %tid, %c2048 : index
        %_sv8 = memref.load %input[%_si8] : memref<3072xf16, strided<[1], offset: ?>, #gpu.address_space<global>>
        memref.store %_sv8, %smem[%_si8] : memref<3072xf16, #gpu.address_space<workgroup>>
        %_si9 = arith.addi %tid, %c2304 : index
        %_sv9 = memref.load %input[%_si9] : memref<3072xf16, strided<[1], offset: ?>, #gpu.address_space<global>>
        memref.store %_sv9, %smem[%_si9] : memref<3072xf16, #gpu.address_space<workgroup>>
        %_si10 = arith.addi %tid, %c2560 : index
        %_sv10 = memref.load %input[%_si10] : memref<3072xf16, strided<[1], offset: ?>, #gpu.address_space<global>>
        memref.store %_sv10, %smem[%_si10] : memref<3072xf16, #gpu.address_space<workgroup>>
        %_si11 = arith.addi %tid, %c2816 : index
        %_sv11 = memref.load %input[%_si11] : memref<3072xf16, strided<[1], offset: ?>, #gpu.address_space<global>>
        memref.store %_sv11, %smem[%_si11] : memref<3072xf16, #gpu.address_space<workgroup>>
        gpu.barrier
        ...
        scf.if %in_bounds {
          %layer_off = arith.muli %layer_idx, %c3342336 : index
          %row_byte_off = arith.muli %row, %c3264 : index
          %row_byte_abs = arith.addi %layer_off, %row_byte_off : index



          %c96 = arith.constant 96 : index
          %scaled = scf.for %b = %lane to %c96 step %c32 iter_args(%acc = %cst_zero) -> (f32) {
            ...
            %inner = scf.for %j = %c0 to %c32 step %c1 iter_args(%iacc = %cst_zero) -> (f32) {
              ...
              %iv = memref.load %smem[%k] : memref<3072xf16, #gpu.address_space<workgroup>>
