...
void decode_dispatch_4_matmul_1024x1x1024_f16(
    const uint8_t* __restrict__ q8_stacked,
    const char* __restrict__ b1_raw,
    char* __restrict__ b2_raw,
    uint32_t c0, uint32_t c1, uint32_t c2
) {
    const int N_DIM = 1024, K_DIM = 1024;
    const int Q8_BLOCK = 32, Q8_BYTES = 34;
    const int BYTES_PER_ROW = 1088;
    const long long BPL = 1114112LL;

    // c0 = output byte offset (i32 direct), c1+c2 = layer index (i64)
    long long layer = (long long)c1 | ((long long)c2 << 32);
    const __half* input = (const __half*)(b1_raw + 2112);
