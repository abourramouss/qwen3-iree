...
void decode_dispatch_12_matmul_1024x1x2048_f16(
    const uint8_t* __restrict__ q8_stacked,
    const char* __restrict__ b1_raw,
    char* __restrict__ b2_raw,
    uint32_t c0, uint32_t c1
) {
    const int N_DIM = 1024, K_DIM = 2048;
    const int Q8_BLOCK = 32, Q8_BYTES = 34;
    const int BYTES_PER_ROW = 2176;
    const long long BPL = 2228224LL;

    long long layer = (long long)c0 | ((long long)c1 << 32);
    const __half* input = (const __half*)(b1_raw + 12352);
    __half* output = (__half*)(b2_raw + 2048);
